...
const lines = [
  ...
  `Product ecosystem categories: ${PRODUCT_CATEGORIES.map((category) => `${category.name} (${category.status}) — ${category.detail} Items: ${category.items.join(", ")}`).join(" | ")}`,
  `Current products and technology work: ${PRODUCTS.map((product) => `${product.name} (${product.status}) — ${product.detail}; Technology: ${product.technology}; Application: ${product.application}`).join(" | ")}`,
